...
//				System.out.println("No");
				...
				as[i].setMeet(true);
				as[i].setDay(jieguo[i].day);
				as[i].setHour(jieguo[i].s);
//				System.out.println("Yes" + " " + jieguo[i].day + " " + jieguo[i].s);
	...
	public SmartArrange(int allFreeDays, Affair[] as) {
		this.cnt = allFreeDays;
		this.as = as;
	}

	public Affair[] doArrange() {
		...
		all = new item[tot];
		for (int i = 0; i < tot; i++) {
			all[i] = new item(as[i].getNo(), as[i].getLocation(), as[i].getKind(), as[i].getTakes(),
					as[i].getStartDay(), as[i].getEndDay());
		}
		solve();
		return this.as;
	}
